...
import { providersApi, mcpApi, skillsApi } from '@/api'
import type { McpInfo, SkillSimple, ModelInfo } from '@/api/types'
...
export function ModelInfoDialog({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (aiModelId > 0) {
        try {
          const m = await providersApi.getModel(aiModelId)
          if (m) {
            setModelInfo(m)
      ...
      if (mcpIds?.length > 0) {
        try {
          const mcpList = await mcpApi.getMcpEndpointsByIDs(mcpIds)
          setMcps(mcpList ?? [])
        } catch { setMcps([]) }
      } else {
        setMcps([])
      ...
      if (skillIds?.length > 0) {
        try {
          const skillList = await skillsApi.getSimpleSkillsByIDs(skillIds)
          setSkills(skillList ?? [])
        } catch { setSkills([]) }
      } else {
        setSkills([])
                  ...
                  {modelInfo ? (
                    <>
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-text-1">{modelInfo.displayName}</span>
                      </div>
                      <div className="flex items-center">
