...
Raven 是一款可自部署的 Agent，每位团队成员拥有独立工作空间。它把模型、工具、知识、文件、技能和工作流装配到同一个运行环境里，让 Agent 能接任务、调工具、读资料、写文件、沉淀能力，真正进入团队的日常工程链路。
...
Raven 不是一个 KPI 项目，也永远不会是。每一个功能、每一行代码，哪怕只是一个按钮，都得有它存在的理由，否则就不上线。我们追求的是一个锐利、可靠的工具，而不是一个靠堆砌功能讨好评审委员会的大而全平台。不膨胀，不功利，不做企业级虚荣。就只是一个把一件事做好的团队 Agent 平台。
...
```bash
docker pull docker.1panel.live/8treenet/raven:v0.1.8

docker run -d --restart=always --name raven-agent \
  -p 8000:8000 \
  -e RAVEN_CHINA_MIRROR=1 \
  docker.1panel.live/8treenet/raven:v0.1.8
```
