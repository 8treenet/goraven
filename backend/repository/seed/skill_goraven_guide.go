...
| ` + "`goraven-features`" + ` | 平台提供给用户的功能。自由模式和角色模式 (Persona)，对话中的附件上传和处理（图片/文档自动解析），技能系统（市场安装/共享/Always-On），MCP 集成（Stdio/SSE/HTTP 三种传输），工作空间和 .profile 环境变量注入，团队项目（独立创建，可设置全员开放/仅成员可见），会话分享，Markdown 渲染与 SSE 流式推送，<goraven-chart> <goraven-file> <goraven-upload> <goraven-ref> 标签，系统设置项（迭代/超时/压缩/开关） |
