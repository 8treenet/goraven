// Package main generated by 'freedom new-project goraven'
package main
...
import (
	...
	_ "goraven/backend/repository" //Implicit initialization repository
...
func installDatabase(app freedom.Application) {
	app.InstallDB(func() interface{} {
		...
		if err = repository.SeedOnBoot(db); err != nil {
			freedom.Logger().Fatal(err.Error())
		}
		return db
	})
...
func serveSharePage(ctx iris.Context) {
	...
	if idx := strings.Index(page, "<title>"); idx >= 0 {
		...
		if end >= 0 {
			page = page[:idx] + "<title>" + html.EscapeString(data.Title) + " · GoRaven</title>" + page[idx+end+len("</title>"):]
		}
	}
